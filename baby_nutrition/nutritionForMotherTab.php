...
<?php
	if(isset($_POST["questionNfm"])) {
		$pat_name = $_SESSION["username"];
		$question = $_POST["question"];
		$question = mysqli_real_escape_string($connection,$question);
		
		$insertNfm = "INSERT INTO nutrition_for_mother (pat_name,question) ";
		$insertNfm .= "VALUES ('{$pat_name}' , '{$question}')";	
		
		$result = mysqli_query($connection,$insertNfm);
		header("location: patientForum.php");
	}
?>

<!-------If disease is checked------>
<?php if($_SESSION["nfm"] == "nfm") { ?>
	...
	<form class="question" style="float:left;margin-top: 43px;margin-left: 55px;" action="patientForum.php" method="post">
		...
		<br/><br/><br/><br/>
		<input class="btn btn-primary btn-md" style="float:left; margin-left:625px;" type="submit" name="questionNfm" value="প্রশ্ন">
	</form>
