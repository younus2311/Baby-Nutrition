...
<?php
	if(isset($_POST["questionHom"])) {
		$pat_name = $_SESSION["username"];
		$question = $_POST["question"];
		$question = mysqli_real_escape_string($connection,$question);
		
		$insertHom = "INSERT INTO health_of_mother (pat_name,question) ";
		$insertHom .= "VALUES ('{$pat_name}' , '{$question}')";	
		
		$result = mysqli_query($connection,$insertHom);
		header("location: patientForum.php");
	}
?>

<!-------If disease is checked------>
<?php if($_SESSION["hom"] == "hom") { ?>
	...
	<form class="question" style="float:left;margin-top: 43px;margin-left: 55px;" action="patientForum.php" method="post">
		...
		<br/><br/><br/><br/>
		<input class="btn btn-primary btn-md" style="float:left; margin-left:625px;" type="submit" name="questionHom" value="প্রশ্ন">
	</form>
...
<div class="comment-holder" style="float:left;margin-top: 20px;margin-left: -27px;">
	<?php
		$questionSelect = "SELECT * FROM health_of_mother ORDER BY id DESC";
